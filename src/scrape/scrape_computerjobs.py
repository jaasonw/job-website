...
class ComputerjobsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        urls = [
            "https://www.computerjobs.com/us/en/JobListing.aspx?shid=97A7F115B70CEA379778"
        ]
        for url in urls:
    ...
    def parse_job_list(self, response):
        job_list = [
            ...
            for url in response.xpath(
                "/html/body/form//div[@class='jobListItem newjobsum']/div[1]/a[@class='jobListPosition']"
            )
            .xpath("@href")
            .getall()
        ]
    ...
    def parse_job(self, response):
        ...
        description = " ".join(
            response.xpath("//*[@id='md_skills']").css("*::text").extract()
        )
        date_posted = response.xpath("//*[@id='md_posted_date']/text()").get().strip()
        location = response.xpath("//*[@id='md_location']/text()").get().strip()
        ...
        result = {
            "url": response.url,
            "title": response.xpath("//*[@id='td_jobpositionnolink']/text()")
            .get()
            .strip(),
            "company": contact,
